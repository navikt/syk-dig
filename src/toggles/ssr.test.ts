...
describe('handleSessionId', () => {
    it('should use have pid when user has auth header and cookie exists', () => {
        const req = {
            ...
            cookies: { 'syk-dig-unleash-session-id': 'unleash-id' } as NextApiRequestCookies,
    ...
    it('should generate new syk-dig-unleash-session-id when no pid and no cookie', () => {
        ...
        expect(setHeaderMock).toHaveBeenCalledWith('set-cookie', [`syk-dig-unleash-session-id=${sessionId}; path=/;`])
    })
})
