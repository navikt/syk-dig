...
import { BodyShort, DatePicker, RangeValidationT, useRangeDatepicker } from '@navikt/ds-react'
    ...
    return (
        <div className="max-w-96">
            ...
            {(fomField.value || tomField.value) && (
                <BodyShort>
                    {fomField.value && <span>{formatsmregDate(fomField.value)}</span>}
                    <span> - </span>
                    {tomField.value && <span>{formatsmregDate(tomField.value)}</span>}
                </BodyShort>
            )}
        </div>
    )
