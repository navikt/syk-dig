...
export function createDefaultValues(oppgave: Oppgave): DefaultValues<NasjonalFormValues> {
    return {
        ...
        medisinskVurdering: {
            hoveddiagnose: {
                system:
                    diagnoseCodeThingyToSystem(oppgave.papirSmRegistering?.medisinskVurdering?.hovedDiagnose?.system) ??
                    'ICD10',
            ...
            bidiagnoser:
                oppgave.papirSmRegistering?.medisinskVurdering?.biDiagnoser.filter(notNull).map((it) => ({
                    system: diagnoseCodeThingyToSystem(it?.system) ?? 'ICD10',
                    code: it?.kode ?? null,
                    text: it?.tekst ?? null,
                })) ?? [],
...
function diagnoseCodeThingyToSystem(diagnosekodeString: string | undefined | null): DiagnoseSystem | null {
